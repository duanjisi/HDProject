...
import com.atgc.hd.base.adapter.holder.AnimateHolder;
...
//            defaultAnimateRemoveImpl(holder);
        }
...
//                        defaultAnimateAddImpl(holder);
    ...
    private void doAnimateRemove(final RecyclerView.ViewHolder holder) {
        if (holder instanceof AnimateHolder) {
            ((AnimateHolder) holder).animateRemoveImpl(holder, new DefaultRemoveVpaListener(holder));
        } else {
    ...
    private void defaultAnimateRemoveImpl(final ViewHolder holder) {
    ...
    private void doAnimateAdd(final RecyclerView.ViewHolder holder) {
        if (holder instanceof AnimateHolder) {
            ((AnimateHolder) holder).animateAddImpl(holder, new DefaultAddVpaListener(holder));
        } else {
...
//        return Math.abs(holder.getAdapterPosition() * getAddDuration() / 4);
    ...
    private void defaultAnimateAddImpl(final ViewHolder holder) {
        final View view = holder.itemView;
        final ViewPropertyAnimator animation = view.animate();
        mAddAnimations.add(holder);
